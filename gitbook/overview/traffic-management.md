...
In exercise 3, we set up some egress rules to allow the guestbook service to call the Watson Tone Analyzer service created in [Exercise 3](https://github.com/moficodes/istio101/tree/12e72c45e55b4a37638f8cd217128b951b989af6/gitbook/exercise-3/README.md). By default Istio blocks calls to services outside the service mesh. In order for calls to reach the Watson service, we applied the `VirtualService` and `ServiceEntry` found in `/istio101/workshop/plans/analyzer-egress.yaml`.
